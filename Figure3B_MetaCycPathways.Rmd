---
title: "TUMS - Results"
subtitle: "Figure 3B - MetaCyc Pathways"
...
# sequencing batch 1: 170 baseline samples and 35 follow-up samples (195 total)

functdata2 <- 'w://Collaborative Projects/HPC_CEPI/ANALYSIS/Project_TUMS/data/microba_batch2/functional_profiles'
# sequencing batch 2: 95 follow-up samples 

# total sequenced samples: 290 (287 unique samples excluding samples repeated for batch effect analysis)
...
```{r}
profiles1 <- read.table(file.path(functdata1, '/by_sample/MetaCyc_pathway.samples.tsv'), header = T, sep = "\t", quote = "")
profiles2 <- read.table(file.path(functdata2, '/by_sample/MetaCyc_pathway.samples.tsv.gz'), header = T, sep = "\t", quote = "")
```
...
```{r}
# sub-set profiles data to only include 18-month samples
...
sigtab <- filter(res2, pvalue < 0.05)
```
...
res2$passed_threshold <- res2$pvalue < threshold

# Sort the data first by padj in ascending order and then by log2FoldChange in descending order
res2 <- res2[order(res2$pvalue, -res2$log2FoldChange), ]

# Create the volcano plot with labels for points above the threshold
volcano_plot <- ggplot(res2, aes(x = log2FoldChange, y = -log10(pvalue))) +
  geom_point(aes(color = passed_threshold), size = 2) +
  geom_text_repel(data = head(subset(res2, passed_threshold)),
                  aes(label = pathway), hjust = 1.2, vjust = 1.2, size = 5) +
...
```{r}
# save plot
#ggsave(file.path(plotdir, 'fig3B.metacyc_pathways.pdf'), device = "pdf", dpi = 300, width = 8, height = 8)
```
